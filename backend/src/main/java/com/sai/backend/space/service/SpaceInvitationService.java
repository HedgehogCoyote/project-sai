...
public class SpaceInvitationService {
	...
	public Long invite(Long inviterUserId, SpaceInvitationRequest spaceInvitationRequest)
	{
		...
		if(spaceInvitationRepository.existsBySpace_IdAndInviter_IdAndInvitee_IdAndStatus(
				space.getId(), inviterUserId, spaceInvitationRequest.inviteeUserId(), SpaceInvitationStatus.PENDING)
	...
	public Long denySpaceInvitation(Long requestUserId, Long invitationId)
	{
		SpaceInvitation spaceInvitation =
				spaceInvitationRepository.findById(invitationId)
	...
	@Transactional
	 public Long joinSpace(Long requestUserId, JoinSpaceRequest joinSpaceRequest) {
		...
		SpaceInvitation spaceInvitation =
				spaceInvitationRepository.findById(joinSpaceRequest.invitationId())
				.orElseThrow(() -> new SpaceInvitationNotFoundException() );
		...
		Space toJoinSpace = spaceRepository
				.findById(spaceInvitation.getSpace().getId())
				.orElseThrow(() -> new SpaceNotFoundException()); 		
		...
		SpaceMember joinedSpaceMember 
			= new SpaceMember(toJoinSpace, toJoinUser, SpaceMemberRole.MEMBER);
	...
	public List<InvitationResponse> getSentInvitationList(Long userId){
		...
		List<SpaceInvitation> invitationListByInviteeId 
			= spaceInvitationRepository.findByInviter_Id(userId);
		
		return invitationListByInviteeId
				.stream()
	...
	public List<InvitationResponse> getReceivedInvitationList(Long userId){
		...
		List<SpaceInvitation> invitationListByInviteeId 
		= spaceInvitationRepository.findByInvitee_Id(userId);
